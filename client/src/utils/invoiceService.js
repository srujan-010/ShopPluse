...
export const invoiceService = {
     ...
     */
    generateInvoicePDF: (data, shop, type = 'SALE', options = {}) => {
        const doc = new jsPDF();
        ...
        if (type === 'GOV_SALE' && data.customerAadhaar) {
            rightColY += 6;
            doc.text(`Aadhaar: ${data.customerAadhaar}`, 120, rightColY);
        }
        ...
        // Watermark overlay
        const isInspectionCopy = options.isInspectionCopy !== undefined 
            ? options.isInspectionCopy 
            : (data && data.isInspectionCopy);

        if (type === 'GOV_SALE' && isInspectionCopy) {
            doc.setFont("helvetica", "bold");
            doc.setFontSize(28);
            doc.setTextColor(252, 165, 165); // faint light red
            doc.text("OFFICIAL INSPECTION COPY", 30, 200, { angle: 45 });
        }
     ...
     */
    shareInvoice: async (data, shop, type = 'SALE', options = {}) => {
        try {
            const doc = invoiceService.generateInvoicePDF(data, shop, type, options);
     ...
     */
    downloadInvoice: (data, shop, type = 'SALE', options = {}) => {
        const doc = invoiceService.generateInvoicePDF(data, shop, type, options);
        const fileName = `Invoice-${data._id.slice(-6).toUpperCase()}.pdf`;
        doc.save(fileName);
    }
